...
func (c *ReleaseController) Post() {
	...
	// 删除上线日志记录
	o := orm.NewOrm()
	_, _ = o.Raw("DELETE FROM `record` WHERE `task_id`= ? ", c.Task.Id).Exec()

	if c.Task.Action == 0 {
		// 生成版本号
		c.makeVersion()
		go func() {
			if err := c.releaseHandling(); err != nil {
				_, _ = models.AddTaskErrLog(&models.TaskErrLog{
					ErrInfo: err.Error(),
					TaskId:  c.Task.Id,
				})
			}
...
func (c *ReleaseController) makeVersion() {
	...
	_ = models.UpdateTaskById(c.Task)
}
...
func (c *ReleaseController) enableRollBack() error {
	...
	if s > 0 && err == nil {
		...
		if s1 > 0 && err == nil {
			...
			s.SetProject(c.Project)
			s.SetTask(c.Task)
			_ = s.CleanUpReleasesVersion(versions)
		}
...
func (c *ReleaseController) failHandling(co *components.BaseComponents) {
	...
	_ = models.UpdateTaskById(c.Task)
	// 清理本地版本库
	_ = co.CleanUpLocal(c.Task.LinkId)
